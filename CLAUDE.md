...
- `apps/api` — NestJS + TypeScript. Bootstrap com `nestjs-pino` (log estruturado), `ValidationPipe` global, filtro global de exceção. `GET /health` para liveness/readiness.
  ...
  - **Modelo de dados atual**: `Company`, `User`, `Membership` (vínculo usuário↔empresa↔papel, multi-empresa), `RefreshToken`, `AuditLog` (`AuditService`, infraestrutura transversal usada por todo módulo futuro), `Provider`/`CommercialAgreement` (Prestadores — cadastro + acordo, spec 4.2, sem MEI/conta PJ/login ainda), `ContractTemplate`/`Contract` (Contratos — geração + versionamento, spec 4.3, sem avaliador de risco de vínculo (fica pra F3) nem assinatura real ainda), `ServiceOrder` (Ordens de Serviço — núcleo do sistema, spec 4.5, máquina de estados `PENDING_MANAGER_APPROVAL → PENDING_FINANCE_APPROVAL → APPROVED`, com `ADJUSTMENT_REQUESTED`/`REJECTED`/`CANCELLED`; valores em campos separados, sem coluna de total; só back-end ainda, sem tela), `ApprovalLevel`/`HierarchyAssignment` (hierarquia de aprovação/visibilidade configurável — sem cargo fixo, árvore de pessoas com `parentId`; `Provider.responsibleAssignmentId` liga cada prestador ao responsável direto), `FinancialEntry`/`FinancialEntryApprovalStep` (Lançamentos Financeiros — pagamentos a prestadores PJ, spec do módulo trazida pelo usuário; máquina de estados `DRAFT → IN_APPROVAL → IN_FINANCE_REVIEW → SCHEDULED → PAID` com `ADJUSTMENT_REQUESTED`/`REJECTED`/`CANCELLED`; cadeia de aprovação "congelada" a partir da `HierarchyAssignment` do prestador no momento do envio; back-end + tela completos, ver `docs/decisions.md`). Financeiro (three-way match, CNAB, Stark Bank/PlugNotas), dados bancários/PIX do prestador, Agendamentos e demais módulos ainda não existem.
