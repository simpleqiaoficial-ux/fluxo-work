...
- `apps/web` — React + Vite + TypeScript. Lint via `oxlint`. Roteamento com `react-router`. Testes com Vitest + Testing Library. `VITE_API_URL` é embutido no build (SPA estática, sem env var em runtime). **Estilo**: Tailwind CSS v4 (`@tailwindcss/vite`) — usar os componentes compartilhados em `src/components/` (`Button`, `Field`/`TextInput`/`Select`, `Card`, `CenteredPage`, `Alert`) em telas novas em vez de estilizar do zero. **Telas atuais**: login, callback, seleção/criação de empresa, layout autenticado (`AppLayout`/`ProtectedRoute`), Prestadores (listar/criar/detalhe com acordos comerciais). Token de acesso só em `AuthContext` (memória), nunca em Web Storage — sessão restaurada via `POST /auth/refresh` a cada carregamento.
